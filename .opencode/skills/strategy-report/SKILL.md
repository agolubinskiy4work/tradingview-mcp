...
1. `data_get_strategy_results` - overall metrics such as net profit, win rate, and profit factor.
...
- Profit Factor, target greater than 1.5.
- Max Drawdown in dollars and percent.
- Average Trade in dollars and percent.
...
- If win rate is less than 50% but profit factor is greater than 1, suggest tighter entries.
- If max drawdown is greater than 20%, suggest position sizing or stop loss adjustments.
- If profit factor is less than 1.2, suggest the strategy may need fundamental changes.
- If there are few trades, suggest widening the lookback or loosening entry criteria.
