...
After adding, use `indicator_set_inputs` to customize settings, such as changing EMA length to 200.
...
- `draw_shape` with `trend_line` for trend channels, using two points.
...
4. `symbol_info` - get symbol metadata.
...
- Indicator readings such as RSI, MACD, and moving averages.
