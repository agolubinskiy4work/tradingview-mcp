...
- What type? Indicator, strategy, or library.
...
If modifying an existing script, run:
...
1. Read the error messages with line number and description.
2. Edit `scripts/current.pine` locally and fix the specific lines.
3. Push again with `node scripts/pine_push.js`.
4. Repeat until there are 0 errors.
...
1. Use `capture_screenshot` to verify it looks right.
2. Use `data_get_strategy_results` if it's a strategy.
...
1. Pull fresh with `node scripts/pine_pull.js` in case TradingView modified anything.
2. Edit locally.
3. Push and compile.
